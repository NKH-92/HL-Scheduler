        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className="rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 p-2 text-white shadow-md shadow-blue-500/30">
                <Layout size={24} />
              </div>
              <div className="min-w-0">
                <h1 className="truncate text-xl font-extrabold tracking-tight text-slate-800">HL-Scheduler</h1>
                <div className="flex flex-wrap items-center gap-2 mt-0.5">
                  <span className="text-[11px] font-bold tracking-wider text-slate-400 uppercase">Project Timeline Studio</span>
                  {versionLabel ? (
                    <span className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-500 ring-1 ring-inset ring-slate-200">
                      v{versionLabel}
                    </span>
                  ) : null}
                </div>
            ...
            <div className="hidden min-w-0 items-center rounded-xl bg-slate-100/80 p-1 md:flex ring-1 ring-inset ring-slate-200/50">
              {canAccessEditor && (
                <TabButton
            ...
            <div className="flex items-center gap-2">
              <div className="flex w-[84px] items-center justify-end gap-2">
                <button
                  ...
                  className={`rounded-xl border border-slate-200 bg-white p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-blue-700 ${isEditMode ? '' : 'pointer-events-none invisible'
                    }`}
                  title="프로젝트 백업(JSON)"
                  type="button"
              ...
              {isAuthenticated ? (
                ...
                <button
                  ...
                  disabled={isAuthBusy}
                  className="rounded-xl bg-blue-600 px-5 py-2 text-sm font-bold text-white shadow-md shadow-blue-500/20 transition-all hover:bg-blue-700 hover:shadow-lg hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isAuthBusy ? '처리 중...' : '로그인'}
                </button>
              )}
          ...
          <div className="hidden md:block">
            <nav
              className={`items-center rounded-2xl border border-slate-200/70 bg-slate-50/80 p-1 md:flex ${isEditMode ? '' : 'pointer-events-none invisible'
                }`}
              aria-hidden={!isEditMode}
            >
              <TabButton
          ...
          <div className="md:hidden">
            ...
            <select
              ...
              disabled={!isEditMode}
              className={`w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 ${isEditMode ? '' : 'pointer-events-none invisible'
                }`}
              aria-hidden={!isEditMode}
              tabIndex={isEditMode ? 0 : -1}
            >
